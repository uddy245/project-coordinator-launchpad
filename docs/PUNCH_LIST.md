...
- [x] **Lesson 20 real video** — narrated-slides RAID-logs video produced via the new `scripts/video/` pipeline (ElevenLabs `eleven_multilingual_v2` voiceover, 25 slides, ~11m56s) and **live in prod 2026-06-24**: uploaded to the Supabase `lesson-videos` bucket at `lesson-20-raid-logs/lesson-20-raid-logs.mp4`, `lessons.raid-logs.video_url` repointed there, `is_published=true`. Native `<video>` path (no Bunny, per founder decision). v2 slide design + apad PAD fix landed in the final build (true 734.8s, ffprobe-verified no truncation). CORRECTION: the earlier "video_url is NULL" note was stale — prod was already serving a 13.8 MB NotebookLM placeholder at the bucket root; that object is preserved as rollback. **Shipped: `CONTENT-VIDEO-001` merged to `main` via PR #88 (squash `c7d94d6`), 2026-06-24; manual ≥90% gate watch-test passed clean.**
...
- 2026-06-24 — **Lesson 20 real video + `scripts/video/` pipeline** (`CONTENT-VIDEO-001`). Reusable narrated-slides pipeline committed under `scripts/video/` (lesson-aware `build_common.py` with the audio-normalization fix + a new ffprobe clip-validation/truncation guard; `render_slides`/`generate_audio`/`build_video`/`make_preview`; per-lesson `lessons/<slug>/segments.py`). Lesson 20 MP4 (ElevenLabs `eleven_multilingual_v2`, v2 slide design, 1920×1080/30fps H.264+AAC, 734.8s, 24.2 MB) uploaded to the Supabase `lesson-videos` bucket and `lessons.raid-logs.video_url` repointed via the idempotent `scripts/seed-production-video.mjs` (new nested key; old NotebookLM placeholder preserved as rollback). The guarded `build_video.py` caught a `-shortest` PAD-truncation bug (clip ended at audio's end, dropping ~18s of breath across the deck — old build had it too); fixed with `-af apad` so the deck renders to its full 734.8s, ffprobe-verified. `.env.example` gains `ELEVENLABS_*`. Unit test `tests/unit/video-segments.test.ts` green (5/5); full unit suite 170/170; typecheck + prettier clean. captions.srt committed but not hosted (bucket is video/* only; no `<track>` consumer yet). **Merged to `main` via PR #88 (squash `c7d94d6`); manual ≥90% gate watch-test passed clean. Admin-merged past a pre-existing red `main` (calibration suite missing Lesson 17's `docs/rubrics/pushback-judgment-v1.json` — unrelated to this PR).**
