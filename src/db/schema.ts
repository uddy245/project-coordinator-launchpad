 ...
 *
 * PULLED, not authored: generated by `drizzle-kit pull` from the migrated
 * database (identical to the old Supabase schema) and lightly edited
 * (tsvector type, numeric -> number, dropped the quiz_items_public view,
 * whose access check depended on auth.uid()). Never push/migrate from this
 * file — the database is the source of truth. Re-run `pnpm db:pull` and
 * diff if the schema changes.
 *
