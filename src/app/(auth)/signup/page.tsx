...
export default async function SignupPage({
  ...
  const sanitisedRef = ref && /^[a-z0-9_-]{1,80}$/i.test(ref) ? ref.toLowerCase() : null;
  ...
  return (
    <div className="space-y-7">
      <div className="space-y-2">
        ...
        <p className="text-sm text-muted-foreground">Begin Module 01 in minutes.</p>
      </div>
      <hr className="section-rule" />
      <SignupForm redirectTo={redirect ?? "/dashboard"} signupSource={sanitisedRef} />
